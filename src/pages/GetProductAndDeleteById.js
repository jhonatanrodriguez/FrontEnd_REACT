...
function GetProductAndDeleteById() {
    ...
    const DeleteProduct = async (id) => {
        console.log("id: -", id);
        try {
            const response = await fetch("http://localhost:3000/product".concat("/") + id, {
                method: "DELETE",
                            ...
                            return (
                                <tr key={i + 1} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    ...
                                    <td class="px-5 py-4 flex flex-row">
                                        <Link to={`/updateProducts/${item.id}`} class="bg-yellow-300 px-5 py-1 my-5 rounded-lg text-black
                                    hover:bg-yellow-400 hover:font-bold mr-1" >Edit</Link>
                                        <Link to={`/getProducts/${item.id}`} class="bg-orange-500 px-5 py-1 my-5 rounded-lg text-white
                                    hover:bg-orange-600 hover:font-bold  mr-1">Get</Link>
                                        <button onClick={() => DeleteProduct(item.id)} class="bg-gray-500 px-5 py-1 my-5 rounded-lg text-white
